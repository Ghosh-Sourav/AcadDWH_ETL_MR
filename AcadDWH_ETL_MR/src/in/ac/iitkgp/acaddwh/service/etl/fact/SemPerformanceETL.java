...
public class SemPerformanceETL implements ETLService<SemPerformance> {
	...
	public static class ETMapper extends Mapper<Text, Text, Text, Text> {
		...
		public void map(Text key, Text value, Context context) throws IOException, InterruptedException {
			...
			/* Extract: BEGIN */
			semPerformance.setSplKey(key.toString());
			semPerformance.setStudentKey(itr.nextToken());
			semPerformance.setCgpa(Float.parseFloat(itr.nextToken()));
			semPerformance.setCourseRegistered(Integer.parseInt(itr.nextToken()));
			semPerformance.setCreditRegistered(Integer.parseInt(itr.nextToken()));
			semPerformance.setCourseFailed(Integer.parseInt(itr.nextToken()));
			/* Extract: END */
			...
			attributes.set(semPerformance.getPrintableLineWithoutKeyAndNewLine());
			context.write(new Text(semPerformance.getInstituteKey()), attributes);
		}
	}
	...
	public void load(String hdfsFilePath, String absoluteLogFileName) throws LoadException {
		...
		try {
			semPerformanceDAO.addToHive(con, hdfsFilePath);
			System.out.println("[H] Loaded SemPerformance file: " + hdfsFilePath);
